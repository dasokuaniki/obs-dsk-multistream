...
- Automatic integration with a separately installed DSK Comment Viewer. The browser dock appears only after the Viewer's loopback v1 API passes validation.
...
The generated `release\DSK-Multistream-<version>-Windows-x64-Setup.exe` installs the plugin for all users and registers it in Windows **Settings > Apps > Installed apps**. Running a newer installer updates the same installation. Uninstall removes only the dedicated plugin binaries and static data; OBS profiles, DSK settings, and credentials are intentionally preserved.
